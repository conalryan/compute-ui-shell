 ...
 *
 * Fallback: `{appBase}/entry.js` for local/dev or older publishes.
 */
export async function resolveEntryUrl(appName: string): Promise<string> {
  const base = appBase(appName);
  ...
  if (manifest?.entry) {
    ...
    // relative path e.g. assets/entry.abc123.js
...
async function fetchDeployManifest(url: string): Promise<AppDeployManifest | null> {
  try {
...
async function waitForCustomElement(tagName: string, timeoutMs = 5_000): Promise<boolean> {
 ...
 */
export async function loadAndMountApp(appName: string, container: HTMLElement): Promise<LoadedApp> {
  ...
  const mod = (await import(/* @vite-ignore */ entryUrl)) as MicrofrontendModule;
...
export async function unmountApp(loaded: LoadedApp | null, container: HTMLElement): Promise<void> {
